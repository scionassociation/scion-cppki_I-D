...
The TRC also provides *trust agility* - enabling relying parties (endpoints and ASes) to select the trust roots used to initiate certificate validation.
For endpoints, this implies that users are free to choose ISDs trusted when verifying path segments. For ASes, this implies that they can decide whether to trust other ISDs for beaconing, thus transparently defining trust relationships between parts of the network. The SCION trust model therefore, differs from the one provided by other PKI architectures.
